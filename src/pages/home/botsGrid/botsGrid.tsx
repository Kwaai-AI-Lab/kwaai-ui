...
const BotsGrid: React.FC<BotsGridProps> = ({
    ...
    const {
        ...
        refetchAssistants,
    } = useAssistants('assistant');
    const {
        sharedAssistants,
        loading: loadingSharedAssistants,
    ...
    const canCreateAgent = () => {
        return (
            (viewType === AgentViewType.MyAgents ||
                viewType === AgentViewType.SharedAgents) &&
            personas.length === 0
        );
    };
    ...
    const handleBotDelete = async (botId: string) => {
        try {
            ...
            setLocalItems((prevItems) =>
                prevItems.filter((item) => item.id !== botId)
            );
        } catch (error) {
        ...
        <>
            ...
            <BotListTitle
                ...
                onAddNewAgent={() => {
                    if (canCreateAgent()) {
                        toast.error(
                            'Cannot create an assistant: please create a persona first'
                        );
                    } else {
                        handleAddNewAgent();
                    }
