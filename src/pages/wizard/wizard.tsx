...
const Wizard: React.FC<WizardProps> = ({
    ...
    const handleMessage = async (inputValue: string): Promise<string> => {
        try {
            ...
            const answer = await assistantsService.getAnswer(
                newBot.id || '',
                inputValue
            );
            return answer;
            ...
            if (currentStep === 2 && !isUpdateMode) {
                response = await assistantsService.createAssistant(
                    ...
                    newBot.kind,
                    {
                        ...
                        allow_edit:
                            newBot.allow_edit === 'True' ? 'True' : 'False',
                        icon: newBot.icon,
    ...
    const handleDeploy = async () => {
        try {
            const assistantsService = new AssistantsService();
            await assistantsService.updateAssistant(newBot.id || '', {
                ...newBot,
