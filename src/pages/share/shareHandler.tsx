...
function ShareHandler() {
    ...
    const personaImages: { [key: string]: string } = {
        '7bea4732-214f-40e7-9161-4e7241a2b97e':
            'https://static.vecteezy.com/system/resources/previews/026/536/284/non_2x/27yr-old-beautiful-face-ai-generated-free-photo.jpg',
        '7bea4732-214f-40e7-9161-4e7241a2b97f':
            'https://img.freepik.com/premium-photo/face-that-has-word-ai-it_872754-2069.jpg',
        '7bea4732-214f-40e7-9161-4e7241a2b97a':
    ...
    const personaImageUrl =
        persona && personaImages[persona.face_id]
            ? personaImages[persona.face_id]
            : '/default-image.png';
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (shareId) {
                if (userId !== null) {
                    try {
                        await AssistantsService.updateShareAssistant(
                            shareId,
                            userId
                        );
                    } catch (error) {
                ...
                try {
                    const shareDetails =
                        await AssistantsService.getShareDetails(shareId);
        ...
        const fetchAssistantAndPersona = async () => {
            ...
            try {
                const assistantResponse =
                    await AssistantsService.getAssistant(assistantId);
                ...
                if (assistantResponse.persona_id) {
                    try {
                        ...
                        const fetchedPersona = await personasService.getPersona(
                            assistantResponse.persona_id
                        );
    ...
    const handleMessages = async (inputValue: string): Promise<string> => {
        ...
        try {
            ...
            const { messageData, audioUrl } =
                await messagesServiceInstance.sendMessage(
                    assistantId,
                    null,
                    inputValue,
                    isMuted ? 'False' : 'True',
            ...
            setMessages((prevMessages) =>
                prevMessages.map((msg) =>
                    msg.id === userMessage.id
                        ? { ...msg, chat_response: messageData.chat_response }
                        : msg
                )
            );
            ...
            if (audioUrl) {
                ...
                await audio.play();
                await messagesServiceInstance.deleteAudioMessage(
                    messageData.id
                );
            }
    ...
    const mappedMessages = Array.isArray(messages)
        ? messages.flatMap((message) => {
              ...
              return mappedBotMessage
                  ? [mappedUserMessage, mappedBotMessage]
                  : [mappedUserMessage];
          })
        : [];
